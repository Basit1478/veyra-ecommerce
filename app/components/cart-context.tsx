...
type CartValue = { lines: CartLine[]; open: boolean; count: number; hydrated: boolean; announcement: string; setOpen: (v: boolean) => void; add: (p: Product, reveal?: boolean) => void; change: (s: string, d: number, tone?: string) => void; clear: () => void };
...
export function CartProvider({ children }: { children: React.ReactNode }) {
  ...
  const value = useMemo<CartValue>(() => ({ lines, open, hydrated, announcement, setOpen, count: lines.reduce((s,l)=>s+l.quantity,0),
    ...
    change(slug,delta,tone){setLines((c)=>c.map((l)=>l.slug===slug&&(!tone||l.tone===tone)?{...l,quantity:l.quantity+delta}:l).filter((l)=>l.quantity>0));}, clear(){setLines([]);setOpen(false);} }), [lines,open,hydrated,announcement]);
